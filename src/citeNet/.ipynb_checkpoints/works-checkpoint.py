...
class Works:
    ...
    def get_referenced_works(self):
        """Retrieve information about works that are referenced by this work."""
        doireferenced = []
        titlereferenced = []
        oaidreferenced = []
        journalreferenced = []
            ...
            for j in self.data["referenced_works"]:
                try:
                    ...
                    if req_referenced.status_code == 200:
                        ...
                        if (
                            referenced_data["primary_location"]
                            and referenced_data["doi"]
                            and referenced_data["title"]
                            and referenced_data["id"]
                            and referenced_data["primary_location"]["source"]
                        ):
                            ...
                            journalreferenced.append(
                                referenced_data["primary_location"]["source"][
                                    "display_name"
                                ]
                            )

                        print(
                            f"Failed to fetch referenced work data for OAID {j}. Status code: {req_referenced.status_code}"
                        )
                except requests.exceptions.RequestException as e:
    ...
    def build_cite_network(self, G, depth):
        ...
        if depth > 0:
            ...
            for doi, title, journal in zip(
                doireferenced, titlereferenced, journalreferenced
            ):
                G.add_node(title, journal=journal, doi=doi)
            for doi, title, journal in zip(
                cited_by_doi, cited_by_title, cited_by_journal
            ):
                G.add_node(title, journal=journal, doi=doi)

            edgelist = [
                (title_cited_by, self.data["title"])
                for title_cited_by in cited_by_title
            ]
            edgelist += [
                (self.data["title"], ref_title) for ref_title in titlereferenced
            ]
    ...
    def plot_cite_network(self, max_depth):
        ...
        color_map = {
            journal: plt.cm.tab20(i) for i, journal in enumerate(unique_journals)
        }
        ...
        node_trace = go.Scatter(
            ...
            hoverinfo="text",
            text=[
                f"<br> <a href='{doivals[node]}' target='_blank'> Node: '{node}' Journal: '{journalnames[node]}'</a>"
                for node in G.nodes()
            ],
        )
        ...
        layout = go.Layout(
            ...
            annotations=[
                ...
                for (x0, y0), (x1, y1) in [
                    (pos[edge[0]], pos[edge[1]]) for edge in G.edges()
                ]
            ],
        )
